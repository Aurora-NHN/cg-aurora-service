...
public interface ProductRepository extends PagingAndSortingRepository<Product,Long> {
    @Query("SELECT p FROM Product p WHERE p.isDelete = true AND p.isActivated = true ")
    Page<Product> findAll(Pageable pageable);
    @Query("SELECT p FROM Product p WHERE p.isDelete = true AND p.isActivated = true AND LOWER(p.name) LIKE %:keyWord%")
