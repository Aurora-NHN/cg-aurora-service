...
public class ImageServiceImpl implements ImageService {
    ...
    public void init(ApplicationReadyEvent event) {
        try {
            ClassPathResource serviceAccount = new ClassPathResource("aurora-firebase.json");
    ...
    public List<String> getImageUrls(List<String> filenames) {
        return filenames.stream().map(this::getImageUrl).collect(Collectors.toList());
    }
    ...
    public List<String> save(List<MultipartFile> files) throws IOException {
        List<String> filenames = files.stream().map(this::save).collect(Collectors.toList());
        List<String> filteredFileName = filenames.stream().filter(Objects::nonNull).collect(Collectors.toList());
